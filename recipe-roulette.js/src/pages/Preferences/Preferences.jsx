...
export function useLocalSave() {
    ...
    if (localStorage.getItem(local_filter) === null) {
        console.log("no local save")
    } else {
...
export function Preferences() {
    ...
    const { handleDeselectAll, ingredients, setIngredients, toggleFilter, filter } = useManageIngredients()
    ...
    useEffect(() => {
        if (local_filter) {
            if (local_filter.isGlutenFree !== filter.isGlutenFree) {
                toggleFilter(isGlutenFree)
            }
            if (local_filter.isVegetarian !== filter.isVegetarian) {
                toggleFilter(isVegetarian)
            }
            if (local_filter.isVegan !== filter.isVegan) {
                toggleFilter(isVegan)
            }

            if (local_recipeFilter.isGlutenFree !== recipeFilter.isGlutenFree) {
                toggleRecipeFilter(isGlutenFree)
            }
            if (local_recipeFilter.isVegetarian !== recipeFilter.isVegetarian) {
                toggleRecipeFilter(isVegetarian)
            }
            if (local_recipeFilter.isVegan !== recipeFilter.isVegan) {
                toggleRecipeFilter(isVegan)
            }

            // setIngredients( prev => ({...prev, blackListed: local_blacklist}))
    ...
    function handleApply(e) {
        if (e.target.checked === true) {
            if (filter.isGlutenFree !== recipeFilter.isGlutenFree) {
                toggleRecipeFilter("isGlutenFree")
            }
            if (filter.isVegetarian !== recipeFilter.isVegetarian) {
                toggleRecipeFilter("isVegetarian")
            }
            if (filter.isVegan !== recipeFilter.isVegan) {
                toggleRecipeFilter("isVegan")
            }
        }
    }
    ...
    function handleSave() {
        ...
        localStorage.setItem("local_blacklist", JSON.stringify(ingredients.blacklisted))
    ...
    return (
        <div className={`${classes.preferencesPage} ${animate && classes.animateSettings}`}>
            <p className={classes.description}>the preferences you set here will affect both ingredients and recipes results</p>
            ...
            <div>
                <label>
                    <input type="checkbox" onClick={handleApply} /> same for recipes?
                </label>
            </div>
            <br />
            <div className={classes.bottomButtons}>
                ...
                />
                <button type="button" className={classes.button} onClick={handleSave} width={"fill"}>
                    Save
                </button>
            </div>
        </div>
