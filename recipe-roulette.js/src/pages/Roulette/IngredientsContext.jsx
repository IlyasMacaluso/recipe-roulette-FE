...
export const IngredientsProvider = ({ children }) => {
  ...
  const {
    ...
    discardBLChanges,
    setDiscardBLChanges,
  } = useIngredientUpdate(ingredients, setIngredients);
  const {
    handleIngDecrement,
    handleIngIncrement,
    shuffleIng,
    generateIngredients,
  } = useDisplayedIngredients(ingredients, setIngredients);
  ...
  const {
    ...
    queryFn: async () =>
      await getRequest("http://localhost:3000/api/ingredients/get-ingredients"),
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await getRequest(
        `http://localhost:3000/api/preferences/get-blacklisted-ingredients/${id}`,
      );
      return res;
    },
    ...
    const initialSetup = async () => {
      ...
      const initIngredientProps = (ings, blIngs, displayedIngs) => {
        return ings.map((ingredient) => ({
          ...ingredient,
          is_blacklisted: blIngs
            ? blIngs.some((id) => id === ingredient.id)
            : ingredient.is_blacklisted,
          is_selected: displayedIngs
            ? displayedIngs.some((id) => id === ingredient.id)
      ...
      if (DBIngredients) {
        setIngredients((prev) => {
          ...
          const selectedIds = localIngredients?.displayed?.map((ing) =>
            ing.is_selected ? ing.id : null,
          );

          const all = initIngredientProps(
            DBIngredients,
            blacklistedIds,
            selectedIds || [],
          );
          const displayed = initIngredientProps(
            localIngredients?.displayed || [],
            blacklistedIds || [],
          );
  ...
  useEffect(() => {
    ...
    setIngredients((prev) => {
      if (prev?.all) {
        ...
        const filterIngredients = (prop) =>
          (filtering = filtering.filter((item) => item[prop]));
...
export const useManageIngredients = () => {
  ...
  if (!context) {
    throw new Error(
      "useManageIngredients must be used within an IngredientsProvider",
    );
  }
  return context;
};
